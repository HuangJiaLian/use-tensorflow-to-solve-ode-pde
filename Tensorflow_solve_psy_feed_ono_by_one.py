...
cs = tf.placeholder(tf.float32, [None, 1])

# define hidden layer and output layer
l1 = add_layer(xs, 1, 50, actication_function = tf.nn.sigmoid)
...
net_out = add_layer(l1, 50, 1, actication_function = None)
...
d_psy_t = tf.gradients(ys=psy_t, xs=xs)
...
loss = tf.reduce_sum(tf.square(d_psy_t - func))
...
for i in range (4000000):
    ...
    data = random.rand(1, 1)
    sess.run(train_step, feed_dict={xs:data, cs:c_space})
    if i % 100 == 0:
        print(sess.run(loss,feed_dict={xs:data, cs:c_space}))
        if not os.path.exists(MODEL_SAVE_PATH):
            os.makedirs(MODEL_SAVE_PATH)
        saver.save(sess, os.path.join(MODEL_SAVE_PATH, MODEL_NAME))
        # Important 
        res = sess.run(psy_t, feed_dict={xs:x_space, cs:c_space})
        try:
